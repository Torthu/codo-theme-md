...
module.exports = class Theme.Theme
	...
	pathFor: (kind, entity, prefix='') ->
		...
		switch kind
			...
			when 'class', 'mixin'
				prefix + kind + '/' + entity.name.replace(/\./, '/') + '.md'
	...
	renderClasses: ->
		@render 'index', 'class/index.md', entity: TreeBuilder.build @environment.visibleClasses(), (klass) ->
			[klass.basename, klass.namespace.split('.')]
	...
	renderMixins: ->
		@render 'index', 'mixin/index.md', entity: TreeBuilder.build @environment.visibleMixins(), (klass) ->
			[klass.basename, klass.namespace.split('.')]
	...
	renderExtras: ->
		@render 'index', 'extra/index.md', entity: TreeBuilder.build @environment.visibleExtras(), (extra) ->
